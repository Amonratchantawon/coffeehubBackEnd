...
describe('Promotion CRUD tests', function () {
  ...
  it('middleware read Pomotions by Shop', function (done) {
    ...
    p2.save(function () {
      agent.get('/api/pomotions/' + shop._id)
        .end(function (pomotionsGetErr, pomotionsGetRes) {
          ...
          if (pomotionsGetErr) {
            return done(pomotionsGetErr);
          }
          // Get Aps list
          var pomotions = pomotionsGetRes.body;
          //console.log('rout.test>>>>>>>>>>>>>>>>>>>>>>>>' + JSON.stringify(pomotions));
          // Set assertions
          (pomotions.length).should.match(2);
          (pomotions[0].name).should.equal('Promotion Name');
          (pomotions[0].code).should.equal('A5468Yu');
          (pomotions[0].detail).should.equal('Open new shop discount 5%');
          // (pomotions[0].name).should.equal('Promotion Name');
          //(pomotions[0].name).should.equal(name);
          // Call the assertion callback
          done();
        });
    });
